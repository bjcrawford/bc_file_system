...
int main(int argc, char **argv)
{
	...
	createDirectory(&vd, "directory0");

	fprintf(stdout, "Opening 'directory0/file1.txt'\n");
	BC_FILE *file1 = openFile(&vd, "directory0/file1.txt");

	fprintf(stdout, "\nroot/ directory listing: \n\n%s\n", getDirectoryListing(&vd, "root"));
	fprintf(stdout, "directory0/ directory listing: \n\n%s\n", getDirectoryListing(&vd, "directory0"));
	
	printBootClusterInfo(&vd);

	fprintf(stdout, "Opening 'one/two/three/file2.txt'\n");
	BC_FILE *file2 = openFile(&vd, "one/two/three/file2.txt");

	fprintf(stdout, "\nroot/ directory listing: \n\n%s\n", getDirectoryListing(&vd, "root"));
	fprintf(stdout, "directory0/ directory listing: \n\n%s\n", getDirectoryListing(&vd, "directory0"));
	fprintf(stdout, "one/ directory listing: \n\n%s\n", getDirectoryListing(&vd, "one"));
	fprintf(stdout, "one/two/ directory listing: \n\n%s\n", getDirectoryListing(&vd, "one/two"));
	fprintf(stdout, "one/two/three/ directory listing: \n\n%s\n", getDirectoryListing(&vd, "one/two/three"));
	...
	writeFile(&vd, test644, 645, file0);
	...
	writeFile(&vd, test2066, 2066, file1);
	...
	bc_rewind(file2);
	char strRead2[70];
	readFile(&vd, strRead2, 0, 69, file2);
	strRead2[69] = '\0';
	printf("%s\n\n", strRead2);
	...
	bc_rewind(file0);
	char strRead0[646];
	readFile(&vd, strRead0, 0, 645, file0);
	strRead0[645] = '\0';
	printf("%s\n\n", strRead0);
